...
{
	SpLib::ILcTexture*	p = NULL;

	if(FAILED(SpLib::LcDev_TextureCreate( "File", &p, sFileName, &dc)))
		return -1;

	SpLib::m_vTex.push_back(p);
...
INT SpLib_TextureRelease(INT _nID)
{
	INT iSize = SpLib::m_vTex.size();
	INT	nIdx=-1;

	for(INT i=0; i<iSize; ++i)
	{
		if(SpLib::m_vTex[i]->GetID() == _nID)
		{
			nIdx = i;
			break;
		}
	...
	itTx = SpLib::m_vTex.begin() + nIdx;
	SAFE_DELETE(	SpLib::m_vTex[nIdx]	);
	SpLib::m_vTex.erase(itTx);

	iSize = SpLib::m_vTex.size();

	return iSize;
}

INT SpLib_TextureWidth(INT _nID)
{
	INT iSize = SpLib::m_vTex.size();
	INT	nIdx=-1;
	...
	return (INT)SpLib::m_vTex[nIdx]->GetImageWidth();
}


INT SpLib_TextureHeight(INT _nID)
{
	INT iSize = SpLib::m_vTex.size();
	INT	nIdx=-1;
	...
	return (INT)SpLib::m_vTex[nIdx]->GetImageHeight();
}
