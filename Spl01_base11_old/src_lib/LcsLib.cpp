				  ...
				  , DWORD dC)
{
	...
	if(0.0F != fAngle)
	{
		...
		if(pRotation)
		{
			D3DXMatrixTranslation(&mtRti, -pRotation->x, -pRotation->y, 0);
			D3DXMatrixTranslation(&mtRt, pRotation->x, pRotation->y, 0);
		}
	}
	...
	hr = LcsLib::m_pd3dSprite->Begin(D3DXSPRITE_ALPHABLEND);

		hr = LcsLib::m_pd3dSprite->SetTransform(&mtW);
		hr = LcsLib::m_pd3dSprite->Draw(pTx, pSrcRect, NULL, NULL, dC);

	D3DXMatrixIdentity(&mtW);
	LcsLib::m_pd3dSprite->SetTransform(&mtW);

	hr = LcsLib::m_pd3dSprite->End();
...
INT LcsLib_CreateWin(INT x,INT y,INT ScnW,INT ScnH,char* sName,INT bFull)
{
	...
	if(FAILED(D3DXCreateSprite(LcsLib::m_pd3dDevice, &LcsLib::m_pd3dSprite)))
	{
		LcsLib::m_pd3dDevice->Release();
		LcsLib::m_pD3D->Release();
		return -1;
	...
	LcsLib::LcDev_ModelInit(LcsLib::m_pd3dSprite);
...
void LcsLib_DestroyWin()
{
	...
	if(LcsLib::m_pd3dSprite)	{	LcsLib::m_pd3dSprite->Release();	LcsLib::m_pd3dSprite = NULL;	}
